...
main :: proc() {
    ...
    for (!glfw.WindowShouldClose(window_handle)){
        ...
        if glfw.GetKey(window_handle, glfw.KEY_R) == glfw.PRESS{
        ...
        if glfw.GetKey(window_handle, glfw.KEY_SPACE) == glfw.PRESS{
            camera_pos += camera_speed * camera_up;
        }
        if glfw.GetKey(window_handle, glfw.KEY_C) == glfw.PRESS{
            camera_pos -= camera_speed * camera_up;
        }
        ...
        light_model *= glm.mat4Scale(glm.vec3{0.2,0.2,0.2}) // make the light cube smaller
